# Compute the paper's QSAR roughness stats (Spearman, AD-partial, cliff AUC, DL gap, mixed-effects).
import os, sys
...
df["gap"] = df["gnn_err"] - df["rf_err"]
print(f"Loaded {len(df)} test compounds across {df.dataset.nunique()} targets "
      f"({df.cliff_mol.mean()*100:.1f}% cliffs)\n")

LANDSCAPE = ["dirichlet", "lipschitz"]
APRIORI   = ["nbr_disp", "sali_max", "sali_mean", "holder"]
BASELINES = ["nn_sim", "local_dens", "rf_var", "mol_size"]
