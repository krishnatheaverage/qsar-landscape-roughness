...
Two lines of work come closest, and stating our claim against them is essential. Sheridan introduced three per-compound domain-applicability measures for random forests, among them the variance across the ensemble's trees and the predicted value itself, and showed they track expected error.\cite{sheridan2012} The reliability-density neighbourhood method of Aniceto and co-workers likewise maps predictive reliability across chemical space from a query's local neighbourhood, combining distance to the training data with the local density of errors.\cite{aniceto2016} Both are genuinely per-instance, and both, like the applicability domain, are organized around proximity to the training data and the model's behaviour there: they estimate how well-supported a prediction is, and they succeed when failures come from sparsity. Our claim is deliberately orthogonal. We isolate one axis, the local roughness of the structure--activity landscape, that is computable from structure without the model or any activity, that targets the dense-region discontinuities those methods are not built to see, and that for activity cliffs is oriented opposite to the applicability domain. Ensemble variance, the strongest of the proximity-based signals, ranks ordinary error well but is no better than chance on cliffs; the contribution here is the complementary axis that captures exactly that failure, and the demonstration that it, not the conventional signals, is the per-compound scale on which prediction intervals stay valid where the landscape is rough.
...
Our central contribution is to identify the per-compound variable on which a QSAR prediction's reliability should be judged in the regime where models actually fail. The reliability signals in use, ensemble uncertainty and the applicability domain, are organized around how well a prediction is supported by nearby training data; both rank ordinary error but miss activity cliffs, which occur among well-supported close analogs. We show that the local roughness of the structure--activity landscape, computed from structure alone, is the signal that captures the cliff failure mode, and that it is the right scale on which to calibrate intervals: a conformal predictor conditioned on roughness keeps 90\% coverage valid across the roughness range and on cliffs, where conditioning on tree variance or on the applicability domain does not. The roughness axis is complementary, not competitive: tree variance ranks ordinary error better, roughness ranks cliffs better, and a combined score inherits both. In support of this, we establish that the roughness signal predicts per-compound error on all 30 targets, survives control for the applicability domain in partial-correlation and mixed-effects analyses, is activity-free and therefore available before assay, holds across three model classes and three distance metrics, and reproduces on two physicochemical benchmarks. The descriptors that use the query compound's own activity correlate with error more strongly (a similarity-weighted local Dirichlet energy reaches a median Spearman of 0.65), but they require the potency being predicted and so bound what local geometry can explain rather than provide a deployable signal. We are explicit that the activity-free effect sizes are modest; their value is being orthogonal to the reliability axes already in use and capturing the failure those axes miss. A graph-convolutional baseline, included only to characterize a standard message-passing model, loses to descriptors in smooth regions rather than at cliffs, where both fail comparably.
