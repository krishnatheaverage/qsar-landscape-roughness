...
\noindent Machine-learning models of molecular potency fail systematically on activity cliffs, pairs of structurally similar molecules with large potency differences, yet give no warning when a prediction lands on one, and existing measures of structure--activity landscape roughness act at the level of an entire data set rather than the individual compound. We introduce a family of per-compound local landscape roughness measures, including an activity-free variant computable before any assay, and evaluate them on thirty bioactivity benchmarks. Local roughness predicts the per-compound error of a random-forest model on every target (median Spearman correlation 0.65) and retains its predictive value after the applicability domain is controlled in partial-correlation and mixed-effects analyses, establishing it as a distinct axis of reliability rather than a proxy for data sparsity. An activity-free descriptor, the pairwise density of the Structure--Activity Landscape Index over a compound's neighbors, flags labeled activity cliffs with a median area under the receiver-operating-characteristic curve of 0.69, whereas applicability-domain and uncertainty signals miss these cliffs or rank them in the wrong direction; in a triage setting, ranking predictions by roughness preferentially recovers them. The result is a model-agnostic, structure-only estimate of where a quantitative structure--activity relationship (QSAR) prediction should and should not be trusted.
