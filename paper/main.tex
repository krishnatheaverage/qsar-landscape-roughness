...
\noindent Machine-learning models of molecular potency fail systematically on activity cliffs, pairs of structurally similar molecules with large potency differences, yet give no warning when a prediction lands on one, and existing measures of structure--activity landscape roughness act at the level of an entire data set rather than the individual compound. We introduce a family of per-compound local landscape roughness measures, including an activity-free variant computable before any assay, and evaluate them on thirty bioactivity benchmarks. Local roughness predicts the per-compound error of a random-forest model on every target and retains its predictive value after the applicability domain is controlled, while an activity-free descriptor flags labeled activity cliffs that applicability-domain and uncertainty signals miss or rank in the wrong direction. The result is a model-agnostic, structure-only estimate of where a QSAR prediction should and should not be trusted.
...
\end{equation}
i.e., the similarity-weighted mean squared activity gap between $i$ and its neighbors; it is the discrete analogue of the squared gradient of the activity function evaluated at $i$. The second is a local Lipschitz constant, an aggregation of the pairwise Structure--Activity Landscape Index (SALI\cite{guha2008}),
\begin{equation}
...
\subsection*{Local landscape roughness predicts per-compound error}

We first asked whether a local geometric measure of landscape roughness predicts per-compound error, treating each target as an independent replicate and reporting the distribution of per-target Spearman correlations across the 30 data sets (Figure~\ref{fig:main}A). The similarity-weighted local Dirichlet energy $R_D$ was strongly and uniformly predictive of error, with a median Spearman $\rho$ of 0.65 (interquartile range 0.59--0.68) that was positive in all 30 targets (false-discovery-rate-adjusted $p = 2.7 \times 10^{-9}$); the Lipschitz/SALI aggregate behaved similarly (median $\rho = 0.45$). Because these two descriptors incorporate the query compound's own activity, their predictive strength is in part mechanistic: a tree ensemble interpolates the activities of nearby training molecules, so a compound whose activity departs sharply from its neighbors' is necessarily mispredicted. This is precisely the mechanism by which cliffs break QSAR, and quantifying it is informative, but it does not by itself constitute a usable signal.
...
We therefore turned to descriptors that do not see the query's activity. The activity-free neighborhood dispersion $D$ was a weaker but entirely consistent predictor (median $\rho = 0.28$, positive in all 30 targets, FDR $p = 2.7 \times 10^{-9}$), exceeding both applicability-domain baselines, nearest-neighbor similarity and local density (median $\rho \approx 0.18$--$0.19$), though falling short of the random-forest tree-variance uncertainty estimate (median $\rho = 0.36$). That an a priori structural quantity tracks error at all, on every target, is the first indication that cliff-prone regions are identifiable before any activity is measured.
