...
To test whether the descriptors yield calibrated prediction intervals, we built split-conformal\cite{norinder2014} 90\% intervals from the held-out per-compound residuals. Within each target the test compounds were partitioned at random into equal calibration and evaluation halves; the nonconformity score was the absolute residual $|\hat{y}_i - y_i|$ for standard conformal prediction, or $|\hat{y}_i - y_i| / \sigma(x_i)$ for a normalized variant whose per-compound scale $\sigma$ was taken from an activity-free roughness (the pairwise-SALI density) or, for comparison, from the applicability domain ($1 - s_{\mathrm{NN}}$). The interval half-width is the rank-$\lceil (n+1)(1-\alpha) \rceil$ calibration score times $\sigma(x_i)$, which retains the finite-sample marginal coverage guarantee at $1 - \alpha = 0.9$ for any $\sigma$. We report marginal coverage, coverage restricted to labeled activity cliffs, and interval width, averaged over 50 random calibration/evaluation splits and aggregated as the median across targets.
...
\subsection*{Calibrated prediction intervals that cover activity cliffs}

The descriptors are diagnostic, but they can also be built into a predictor that issues honest uncertainty. Split-conformal regression\cite{norinder2014} converts a point prediction into an interval with a coverage guarantee, yet that guarantee is only marginal: a constant-width interval covers the easy majority and quietly under-covers the hard minority. On these benchmarks, standard 90\% split-conformal intervals reached 90.4\% coverage overall but only 87.2\% on labeled activity cliffs, against 92.0\% on non-cliff compounds (Figure~\ref{fig:conformal}). Scaling the nonconformity score by the activity-free SALI density, so that intervals widen where the local landscape is rough, raised cliff coverage to 91.2\%, essentially the nominal level, while marginal coverage was held fixed, and it improved cliff coverage on 26 of the 30 targets; the typical interval widened by 15\% (median), with the extra width concentrated on the high-roughness compounds that need it. Scaling instead by the applicability domain, the conventional difficulty estimate, moved cliff coverage the wrong way, down to 82.0\%, because it spends interval width on sparsely sampled regions while cliffs sit among dense analogs. This is the operational form of the orthogonality result: the activity-free roughness supplies exactly the per-compound scale that makes a conformal predictor cliff-aware, and the applicability domain supplies its opposite. Using the activity-dependent Dirichlet energy as the scale over-covers cliffs, which confirms the exploitable structure is there; the point is that the SALI density captures enough of it without ever seeing the activity.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\linewidth]{figure6_conformal.png}
\caption{Roughness-aware conformal prediction intervals. (A) Empirical coverage of 90\% split-conformal intervals on labeled activity cliffs and on non-cliff compounds, for standard intervals (constant width), intervals scaled by the activity-free SALI density, and intervals scaled by the applicability domain ($1 - $ nearest-neighbor similarity); the dashed line is the nominal level. Standard intervals under-cover cliffs, the roughness scale restores nominal cliff coverage, and the applicability-domain scale degrades it further. (B) Median interval width relative to standard, showing the modest width cost of the adaptive scales. Median over 30 targets and 50 calibration/evaluation splits.}
\label{fig:conformal}
\end{figure}
...
Several directions follow. We found the framework to transfer to physicochemical regression, namely aqueous solubility and lipophilicity (Figure~\ref{fig:crossdomain}), and it should extend further to the quantum-mechanical end points for which large computed data sets exist, where the locality of each property could itself be probed. The conformal demonstration here uses a single activity-free scale at one confidence level; richer nonconformity constructions, per-target calibration, and Mondrian schemes conditioned directly on roughness are natural next steps toward prediction intervals that stay valid on cliffs. Stronger activity-free constructs and prospective validation on active discovery campaigns would, finally, test whether structure-only roughness can guide compound triage in practice.
